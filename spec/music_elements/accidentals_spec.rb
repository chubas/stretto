...
describe "attaching accidentals to notes and chords" do
  ...
  context "applying accidental to the base note of a chord" do
    it "should push notes according to the accidental modifier" do
      Stretto::Pattern.new("Cbbmaj").first.notes.map(&:value).should be == [34, 38, 41]
      Stretto::Pattern.new("Cbmaj").first.notes.map(&:value).should  be == [35, 39, 42]
      Stretto::Pattern.new("Cnmaj").first.notes.map(&:value).should  be == [36, 40, 43]
      Stretto::Pattern.new("C#maj").first.notes.map(&:value).should  be == [37, 41, 44]
      Stretto::Pattern.new("C##maj").first.notes.map(&:value).should be == [38, 42, 45]
    end
